...
out = cv2.VideoWriter('outpytag1p1.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (sol_frame_dimen,sol_frame_dimen))
while (cap.isOpened())    :
    #Video Capture
    ...
    list2 = []                      #to store all the contours detected
    contour_counter=0
    for cnt in contours: 
        list1 = [] 
        ...
        if   len(cnt)==4  and    cv2.arcLength(cnt,True)>100    and   cv2.arcLength(cnt,True)<500   : 
...
#        print(list2)
    ...
    #print(list2)   
        ...
        name=str('sol'.join(str(counter_1)))
        #print(name)
        cv2.imshow(name,sol)
        ...
        if identity!=None:
            print([counter_1,identity])  
        #print(list4[0])
        #cv2.putText(frame,identity , list4[0] ,1 ,(255,255,255) , 2) 
